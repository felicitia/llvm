...
# void libft_say(string message_to_user);
# - prints out message_to_user
macro(libft_say message_to_user)
  message(STATUS "LIBFT: ${message_to_user}")
endmacro()

# void libft_warning_say(string message_to_user);
# - prints out message_to_user with a warning
macro(libft_warning_say message_to_user)
  message(WARNING "LIBFT: ${message_to_user}")
endmacro()

# void libft_error_say(string message_to_user);
# - prints out message_to_user with an error and exits cmake
macro(libft_error_say message_to_user)
  message(FATAL_ERROR "LIBFT: ${message_to_user}")
endmacro()

# libft_append(<flag> <flags_list> [(IF_TRUE | IF_FALSE | IF_TRUE_1_0 ) BOOLEAN])
#
# libft_append(<flag> <flags_list>)
#   - unconditionally appends <flag> to the list of definitions
#
# libft_append(<flag> <flags_list> <BOOLEAN>)
#   - appends <flag> to the list of definitions if BOOLEAN is true
#
# libft_append(<flag> <flags_list> IF_TRUE <BOOLEAN>)
#   - appends <flag> to the list of definitions if BOOLEAN is true
#
# libft_append(<flag> <flags_list> IF_FALSE <BOOLEAN>)
#   - appends <flag> to the list of definitions if BOOLEAN is false
#
# libft_append(<flag> <flags_list> IF_DEFINED <VARIABLE>)
#   - appends <flag> to the list of definitions if VARIABLE is defined
#
# libft_append(<flag> <flags_list> IF_TRUE_1_0 <BOOLEAN>)
#   - appends <flag>=1 to the list of definitions if <BOOLEAN> is true, <flag>=0 otherwise
# e.g., libft_append("-D USE_FEATURE" IF_TRUE_1_0 HAVE_FEATURE)
#     appends "-D USE_FEATURE=1" if HAVE_FEATURE is true
#     or "-D USE_FEATURE=0" if HAVE_FEATURE is false
macro(libft_append flags flag)
  if(NOT (${ARGC} EQUAL 2 OR ${ARGC} EQUAL 3 OR ${ARGC} EQUAL 4))
    libft_error_say("libft_append: takes 2, 3, or 4 arguments")
  endif()
  if(${ARGC} EQUAL 2)
  ...
  elseif(${ARGC} EQUAL 3)
    ...
    elseif(${ARGV2} STREQUAL "IF_TRUE_1_0")
      ...
      libft_error_say("libft_append: third argument must be one of IF_TRUE, IF_FALSE, IF_DEFINED, IF_TRUE_1_0")
    endif()
  endif()
endmacro()

# void libft_get_legal_arch(string* return_arch_string);
# - returns (through return_arch_string) the formal architecture
#   string or warns user of unknown architecture
function(libft_get_legal_arch return_arch_string)
  if(${IA32})
  ...
  elseif(${LOONGARCH64})
    ...
    set(${return_arch_string} "${LIBFT_ARCH}" PARENT_SCOPE)
    libft_warning_say("libft_get_legal_arch(): Warning: Unknown architecture: Using ${LIBFT_ARCH}")
  endif()
endfunction()

# void libft_check_variable(string var, ...);
...
function(libft_check_variable var)
  set(valid_flag 0)
  ...
  if(${valid_flag} EQUAL 0)
    libft_error_say("libft_check_variable(): ${var} = ${${var}} is unknown")
  endif()
endfunction()

# void libft_get_build_number(string src_dir, string* return_build_number);
# - grab the eight digit build number (or 00000000) from kmp_version.cpp
function(libft_get_build_number src_dir return_build_number)
...
# void libft_get_legal_type(string* return_legal_type);
# - set the legal type name Performance/Profiling/Stub
function(libft_get_legal_type return_legal_type)
  if(${NORMAL_LIBRARY})
...
# void libft_add_suffix(string suffix, list<string>* list_of_items);
# - returns list_of_items with suffix appended to all items
# - original list is modified
function(libft_add_suffix suffix list_of_items)
  set(local_list "")
...
# void libft_list_to_string(list<string> list_of_things, string* return_string);
# - converts a list to a space separated string
function(libft_list_to_string list_of_things return_string)
  string(REPLACE ";" " " output_variable "${list_of_things}")
  set(${return_string} "${output_variable}" PARENT_SCOPE)
endfunction()

# void libft_string_to_list(string str, list<string>* return_list);
...
function(libft_string_to_list str return_list)
  set(outstr)
